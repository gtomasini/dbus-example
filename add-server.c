...
int main (int argc, char **argv){
    ...
    int ret = dbus_bus_request_name (conn, SERVER_BUS_NAME, DBUS_NAME_FLAG_DO_NOT_QUEUE, &dbus_error);
    ...
    while (1) {
		...
		time(&now);
	        ...
	        const char space [] = " \n\t,";
                printf ("%s msg received from client: \"%s\"\n", ctime(&now), s);
                ...
                if (!error) {        // send reply
                    DBusMessage *reply;
                    char answer [40];
                    sprintf (answer, "server, sum is %ld", i + j);
